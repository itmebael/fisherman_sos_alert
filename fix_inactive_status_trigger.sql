-- Fix trigger to properly handle inactive status
-- This ensures when status is set to 'inactive', it works correctly
...
BEGIN
    ...
    IF NEW.status = 'on_the_way' AND (OLD.status IS DISTINCT FROM NEW.status) THEN
        ...
        BEGIN
            PERFORM public.create_fisherman_notification(
                ...
                jsonb_build_object(
                    ...
                    'casualties', COALESCE(NEW.casualties, 0),
                    'injured', COALESCE(NEW.injured, 0)
                )
            );
        EXCEPTION
            WHEN OTHERS THEN
    ...
    -- Handle "rescued" status (optional)
    IF NEW.status = 'rescued' AND (OLD.status IS DISTINCT FROM NEW.status) THEN
        -- Update resolved_at timestamp if not already set
        IF NEW.resolved_at IS NULL THEN
            NEW.resolved_at = now();
        END IF;
...
COMMENT ON FUNCTION public.notify_fisherman_status_change() IS 'Trigger function that handles status changes and creates notifications. When status is set to inactive, it creates sos_inactive notification.';
