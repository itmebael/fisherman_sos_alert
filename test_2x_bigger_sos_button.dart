...
void main() async {
  ...
  print('=== TEST: 2x Bigger SOS Button with Fixed NoSuchMethodError ===\n');
  ...
  // Test 3: Test SOS alert creation with fixed NoSuchMethodError
  print('\n3. Testing SOS alert creation with fixed NoSuchMethodError...');
  try {
    final position = await Geolocator.getCurrentPosition(
      desiredAccuracy: LocationAccuracy.high,
    );
    ...
    print('   Alert data with fixed NoSuchMethodError:');
    print('   - ID: $alertId');
    ...
    if (response == null) {
      ...
      print('   🔧 This should be handled gracefully now');
  ...
  print('   - 2x bigger size (250x250 instead of 125x125)');
  print('   - Fixed NoSuchMethodError with null response handling');
  print('   - Pulsing animation with smooth scaling');
  print('   - Better error handling for all error types');
  print('   - Improved UI with shadows and borders');
  print('   - Loading state with larger spinner');
  print('✅ Check your app - the SOS button should now be 2x bigger and work without NoSuchMethodError!');
}

Future<void> run2xBiggerSOSButtonTest() async {
  main();
}
